...
export const SERVER_JOB_CATALOG: ServerJobEntry[] = [
  {
    ...
    deadLetter: true,
    recurring: true,
    notes: 'Primary via edge bundle; pg_cron conditional fallback when edge stale',
  },
  {
    id: 'analytics.prune',
  ...
  },
  {
    ...
    deadLetter: true,
    recurring: true,
    notes: 'Primary via webhook/bundle edge; pg_cron conditional fallback; retry_side_effects_dead_letter',
  },
  {
    ...
    deadLetter: false,
    recurring: true,
    notes: 'Edge process-background-queue every ~2min; pg_cron activates when stale',
  },
  {
    id: 'worker.health_audit',
    name: 'Worker health audit',
    trigger: 'pg_cron',
    priority: 'high',
    processor: 'platform_worker_health_audit',
    retryLimit: 1,
    deadLetter: false,
    recurring: true,
    notes: 'Every 5 min via pg_cron worker-health-check job',
  },
  {
