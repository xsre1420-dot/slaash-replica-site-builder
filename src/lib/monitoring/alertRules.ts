...
export const ALERT_RULES: AlertRule[] = [
  ...
  {
    id: 'webhook-worker-stale',
    name: 'Webhook Edge Worker Stale',
    severity: 'critical',
    metric: 'webhook_worker_stale_minutes',
    condition: 'gt',
    threshold: 10,
    windowSec: 600,
    description: 'Webhook edge worker has not succeeded in 10+ minutes while webhooks pending',
    runbook: 'Verify Supabase Edge cron for process-order-webhook-outbox; check BACKGROUND_WORKER_SECRET',
  },
  {
    id: 'background-bundle-stale',
    name: 'Background Bundle Worker Stale',
    severity: 'critical',
    metric: 'background_bundle_stale_minutes',
    condition: 'gt',
    threshold: 10,
    windowSec: 600,
    description: 'Unified background bundle has not succeeded in 10+ minutes while queues pending',
    runbook: 'Verify Edge cron for process-background-queue; pg_cron fallbacks should activate automatically',
  },
  {
    id: 'checkout-failure',
