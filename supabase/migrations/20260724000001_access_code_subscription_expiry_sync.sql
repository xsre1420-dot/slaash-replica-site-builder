-- Sync access-code expiry with subscription end dates; reissue uses remaining term only.

CREATE OR REPLACE FUNCTION public.expire_stale_merchant_access_codes()
RETURNS INTEGER
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WHERE mac.status = 'active'
    AND mac.code_expires_at IS NOT NULL
    AND mac.code_expires_at < NOW();
...
CREATE OR REPLACE FUNCTION public._remaining_subscription_months(p_end TIMESTAMPTZ)
RETURNS INT
LANGUAGE plpgsql
STABLE
SET search_path = public
...
DECLARE
  v_months INT;
BEGIN
  IF p_end IS NULL OR p_end <= NOW() THEN
    RETURN 0;
  END IF;

  v_months := (
    EXTRACT(YEAR FROM age(p_end, NOW())) * 12 +
    EXTRACT(MONTH FROM age(p_end, NOW()))
  )::INT;

  IF EXTRACT(DAY FROM age(p_end, NOW())) > 0 OR EXTRACT(HOUR FROM age(p_end, NOW())) > 0 THEN
    v_months := v_months + 1;
  END IF;
...
-- Heal active codes for converted leads: tie expiry to subscription end.
UPDATE public.merchant_access_codes mac
SET
  subscription_end_at = COALESCE(mac.subscription_end_at, s.end_date),
...
BEGIN
  ...
  IF v_is_reissue THEN
    v_subscription_end_at := public._subscription_end_for_user(v_lead.converted_user_id);
  ...
  ELSE
    v_duration := COALESCE(v_last_duration, v_plan_cap);
  END IF;
...
BEGIN
  ...
  ELSE
    v_subscription_end_at := v_old.subscription_end_at;
    v_code_expires_at := NOW() + INTERVAL '90 days';
    v_duration := v_old.duration_months;
  END IF;
  ...
  LOOP
    ...
    BEGIN
      ...
      VALUES (
        ...
        'active', v_code_expires_at, v_subscription_end_at, v_reason, auth.uid()
      );
      EXIT;
    EXCEPTION
      WHEN unique_violation THEN
...
REVOKE ALL ON FUNCTION public.expire_stale_merchant_access_codes() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.expire_stale_merchant_access_codes() TO service_role;

REVOKE ALL ON FUNCTION public._subscription_end_for_user(UUID) FROM PUBLIC;
REVOKE ALL ON FUNCTION public._remaining_subscription_months(TIMESTAMPTZ) FROM PUBLIC;
