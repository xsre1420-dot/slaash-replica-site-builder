-- v47: Checkout write amplification — one products UPDATE per SKU for variant deductions
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(
    hashtext(v_effective_owner::text || ':' || trim(p_idempotency_key))
  );
  ...
  -- One products UPDATE per distinct SKU (accumulate variant deltas in memory)
  FOR v_product_id IN
    SELECT DISTINCT (item->>'product_id')::UUID
    FROM jsonb_array_elements(p_items) AS t(item)
    WHERE (item->>'quantity')::INT > 0
      AND (
        NULLIF(trim(item->>'selected_size'), '') IS NOT NULL
        OR NULLIF(trim(item->>'selected_color'), '') IS NOT NULL
      )
    ORDER BY 1
  LOOP
    SELECT variants
    INTO v_running_variants
    FROM products
    WHERE id = v_product_id
      AND owner_id = v_effective_owner
    ...
    IF NOT FOUND OR v_running_variants IS NULL THEN
      CONTINUE;
    END IF;
    ...
    UPDATE products p
    SET variants = v_running_variants,
        updated_at = NOW()
...
VALUES (47, 'checkout_variant_consolidation: one products UPDATE per SKU for variant deductions')
