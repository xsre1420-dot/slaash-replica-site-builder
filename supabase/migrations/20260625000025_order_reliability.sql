-- v35: Order reliability — require idempotency key + guarantee unique index

CREATE UNIQUE INDEX IF NOT EXISTS idx_orders_owner_idempotency
  ON public.orders (owner_id, idempotency_key)
  WHERE idempotency_key IS NOT NULL;

COMMENT ON INDEX public.idx_orders_owner_idempotency IS
  'One order per checkout attempt — duplicate submit / retry safety (23505 → recovery)';

-- Patch v28 checkout RPC: mandatory idempotency key + always advisory lock
...
VALUES (35, 'order_reliability: require idempotency key, unique index guarantee')
