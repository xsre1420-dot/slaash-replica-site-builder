...
-- 1. Create a secure function to get current restaurant owner profile without password
...
-- 2. Create a function for password verification (for login purposes only)
...
-- 3. Remove the existing SELECT policy that could expose passwords
...
-- 5. Update the search_path for existing functions to fix security warnings
...
-- 6. Fix the update trigger function search_path
...
-- 7. Fix the customer stats function search_path  
...
-- 8. Fix the user handling function search_path
