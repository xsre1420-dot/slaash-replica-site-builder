-- Converted customers who lost their login code: auto-replace stale/active codes instead of blocking.
...
BEGIN
  ...
  WHERE mac.status = 'active'
    AND (
      (mac.code_expires_at IS NOT NULL AND mac.code_expires_at < NOW())
      OR (mac.subscription_end_at IS NOT NULL AND mac.subscription_end_at < NOW())
    );
