-- Platform health phase 2: checkout atomicity, stock sync, subscription IDOR, inventory RPC

-- ---------------------------------------------------------------------------
-- 1) Stock sync trigger: do not undo checkout deductions
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF current_setting('app.skip_stock_sync', true) = 'on' THEN
    RETURN NEW;
  END IF;
...
-- ---------------------------------------------------------------------------
-- 2) Checkout: rollback on stock failure (RAISE instead of soft RETURN)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3) Subscription check: only current user (close IDOR)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4) platform_schema_version: deny client access
-- ---------------------------------------------------------------------------
ALTER TABLE public.platform_schema_version ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS platform_schema_version_deny ON public.platform_schema_version;
CREATE POLICY platform_schema_version_deny
  ON public.platform_schema_version
  FOR ALL
  USING (false);

-- ---------------------------------------------------------------------------
-- 5) platform_health_check: authenticated only
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 6) Atomic inventory increment (restock)
-- ---------------------------------------------------------------------------
...
VALUES (13, 'platform_health_phase2: checkout rollback, stock sync, subscription IDOR, atomic restock')
