-- Admin leads workflow: stats, filters, quick contact
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'customer_count', (SELECT COUNT(*) FROM public.leads WHERE status = 'customer' OR converted_user_id IS NOT NULL),
    'today_count', (
      SELECT COUNT(*) FROM public.leads
...
CREATE OR REPLACE FUNCTION public.admin_mark_lead_contacted(p_lead_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
REVOKE ALL ON FUNCTION public.admin_leads_stats() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_leads_stats() TO authenticated;

REVOKE ALL ON FUNCTION public.admin_mark_lead_contacted(UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_mark_lead_contacted(UUID) TO authenticated;

REVOKE ALL ON FUNCTION public.admin_list_leads(TEXT, TEXT, TEXT, INT, INT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_list_leads(TEXT, TEXT, TEXT, INT, INT) TO authenticated;
