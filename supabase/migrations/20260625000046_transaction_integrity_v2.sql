-- v46: Transaction integrity — min_stock RPC path (FOR UPDATE) + stricter stock RPC contract
...
BEGIN
  ...
  -- Threshold-only update: single locked UPDATE, no movement row
  IF p_delta <= 0 THEN
...
VALUES (46, 'transaction_integrity: increment_product_stock min-only FOR UPDATE path')
